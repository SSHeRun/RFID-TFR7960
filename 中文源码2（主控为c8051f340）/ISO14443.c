                    ...
                    if(NvBytes > 0)
                    {
                        ...
                        for(i = 0; i < 4; i++)
                        {
                            if(i < (NvBytes - 1))   //将已知的字节和接收到的字节组合成完整的UID码
                                ...
                                send_byte((buf[i + 2 - NvBytes] &~NvBits) | (*(UID + i + 1) & NvBits));
                            else
                                send_byte(buf[i + 2 - NvBytes]);
                        }/* for */
                    }
                    else
                    {
                        send_byte((buf[1] &~NvBits) | (*UID & NvBits));
                        for(i = 0; i < 4; i++)
                        {
                            send_byte(buf[i + 2]);
                        }/* for */
                ...
                for(i = 0; i < 5; i++)
                {
                    completeUID[i + 4] = buf[i + 1];
								...
								for(i = 0; i < 3; i++)          //发送UID等级1
										send_byte(completeUID[i]);
								send_byte(completeUID[3]);       //发送UID1的BCC校验码

								for(i = 4; i < 8; i++)          //发送UID等级2
										send_byte(completeUID[i]);
								send_byte(completeUID[8]);       //发送UID2的BCC校验码
								sendchar(']');
            ...
            for(i = 0; i < 5; i++)
            {
                completeUID[i + 8] = buf[i + 1];
						...
						for(i = 4; i < 7; i++)              //发送UID等级2
								send_byte(completeUID[i]);
						send_byte(completeUID[7]);           //发送UID2的BCC校验码

						for(i = 8; i < 12; i++)             //发送UID等级3
								send_byte(completeUID[i]);
						send_byte(completeUID[12]);          //发送UID3的BCC校验码
						sendchar(']');
...
//    StartCounter;                                   //开始计时
    ...
    for(i = 1; i < 17; i++)                         //1-16个槽轮循
    {
        ...
        {                   	                
						sendchar('[');
						for(j = 1; j < RXTXstate; j++) send_byte(buf[j]);
						sendchar(']');
